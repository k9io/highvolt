...
func LoadConfig(configJSON string) {
	...
	if len(Config.Core.MIME_Types) == 0 {
		...
		l.Logger(l.ERROR, "Cannot find 'core.mime_types' in configation JSON. [%s]", configJSON)
		os.Exit(1)
	...
	if Config.Core.Max_File_Size == 0 {
		...
		l.Logger(l.ERROR, "Cannot find 'core.max_file_size' in configuration JSON.")
		os.Exit(1)

	}

	/* DEBUG: Sanity check Highvolt JSON */
	...
	if Config.Syslog.Host == "" {
		...
		l.Logger(l.ERROR, "Cannot find 'syslog.host' in configation JSON.")
		os.Exit(1)
	...
	if Config.Syslog.Host != "local" {
		...
		if Config.Syslog.Proto == "" {
			...
			l.Logger(l.ERROR, "Cannot find 'syslog.proto' in configation JSON.")
			os.Exit(1)
