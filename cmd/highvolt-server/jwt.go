...
func jwtMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		token, err := jwt.ParseWithClaims(tokenStr, cl, func(t *jwt.Token) (interface{}, error) {
			...
			return models.Env.JWTTokenSecret, nil
		})
		...
		l.Logger(l.NOTICE, "Authentication success for from %s.", c.ClientIP())
...
func authToken(c *gin.Context) {
	...
	ok := authCheck(c.Request.Context(), req.Token)
	...
	expirationTime := time.Now().Add(time.Duration(models.Env.JWTTokenExpire) * time.Minute)
	...
	tokenString, err := token.SignedString(models.Env.JWTTokenSecret)
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"expires_in":   models.Env.JWTTokenExpire * 60, // Convert minutes to seconds (RFC 6749)
	})

	l.Logger(l.INFO, "Got new access token for %s.", c.ClientIP())

}

func authCheck(ctx context.Context, pat string) bool {

	return subtle.ConstantTimeCompare([]byte(models.Env.HIGHVOLT_PAT), []byte(pat)) == 1
